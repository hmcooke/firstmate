...
# (state/x-watch.check.sh), or an agent-to-agent letterbox poll
# (state/letterbox.check.sh), and whether its watcher has a fresh liveness beacon
...
#                         letterbox poll, or a registered event source (a poll or
#                         a source is a wait on something outside this home, not
#                         a task, so neither has task metadata). A letterbox poll
#                         counts because an unanswered peer letter must not wait
#                         for the captain to happen to start a session.
