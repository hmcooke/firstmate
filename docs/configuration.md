...
Cursor submit confirmation follows the [Herdr backend contract](herdr-backend.md#native-agent-state-submit-confirmation), while [runtime backend verification](verification/runtime-backends.md#cursor-agent-cli) owns the current empirical evidence and transcript-state boundary.
