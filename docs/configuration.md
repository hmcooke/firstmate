...
The separate, cheaper endpoint-presence read is tri-state on every backend - present, absent, or unknown, where absence needs positive evidence ([`docs/architecture.md`](architecture.md#endpoint-presence-is-tri-state)); `fm_backend_target_exists` remains its boolean "usable right now" view.
...
A Cursor steer lands on every supported backend, but `fm-send` reports delivery unconfirmed and exits non-zero because Cursor's only stable busy token shares the selected composer row and cannot safely prove submission; [runtime backend verification](verification/runtime-backends.md#cursor-agent-cli) owns the evidence and transcript-state boundary.
