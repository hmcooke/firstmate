...
A run-step the crew must actually drive - a running or fixing step, a gate awaiting its response, or a coarse run whose phase cannot be read - still outranks the declaration, and that reader's header owns the exact rule.
