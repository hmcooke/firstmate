...
That declaration also outranks a run-step that is only monitoring CI: while the pipeline holds a pushed PR open until it is merged or closed, the crew has nothing left to drive, so `bin/fm-crew-state.sh` reports the declared wait as `paused` from the run-step and the status log together, and the same bounded cadence applies instead of a possible-wedge escalation every wedge window.
