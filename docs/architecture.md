...
That declaration also outranks a run-step that is only monitoring CI, including a checks-green or no-CI monitor promoted to ready: while the pipeline holds a pushed PR open until it is merged or closed, the crew has nothing left to drive, so `bin/fm-crew-state.sh` reports the declared wait as `paused` from the run-step and the status log together, retains the wait reason and PR-ready facts in its detail, and applies the same bounded cadence instead of a possible-wedge escalation every wedge window.
A run-step the crew must actually drive - a running or fixing step, a CI monitor whose latest event reports failed checks or active repair, a gate awaiting its response, or a coarse run whose phase cannot be read - still outranks the declaration, and that reader's header owns the exact rule.
