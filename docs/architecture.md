...
For the always-on watcher, a crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
That declaration also outranks a run-step that is only monitoring CI when the latest recognized CI event affirmatively reports waiting or ready, including a checks-green or no-CI monitor promoted to ready: while the pipeline holds a pushed PR open until it is merged or closed, the crew has nothing left to drive, so `bin/fm-crew-state.sh` reports the declared wait as `paused` from the run-step and the status log together and retains the wait reason and PR-ready facts in its detail, after which the watcher applies the same bounded cadence instead of a possible-wedge escalation every wedge window.
A run-step that is active or cannot be proven to be waiting - a running or fixing step, a CI monitor with an unreadable or unrecognized latest event, a CI monitor whose latest event reports failed checks or active repair, a gate awaiting the crew's response, or a coarse run whose phase cannot be read - still outranks the declaration, and that reader's header owns the exact rule.
...
Its initial normal-mode status signal still surfaces through the no-verb path, while away mode self-handles that routine signal and owns the later recheck through its deliberate pre-existing exception: `bin/fm-supervise-daemon.sh` classifies and maintains declared waits directly from the trailing status line via `status_is_paused` and never calls `crew_absorb_class`.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and current code identity, then applies the declared-wait exception above before otherwise keeping that run-step authoritative even if the pane has closed.
The script header owns the exact run-head ancestry rules.
During no-mistakes' `ci` monitor phase, it also reads the ci step log tail because `axi status` reports both "still waiting on checks" and "checks green, waiting on merge" as `ci,running`.
The most recent recognized ci log marker establishes whether that monitor is waiting, ready, or repairing before the declared-wait precedence above is applied.
...
The optional agent-to-agent letterbox rides the same two mechanisms without extending either.
It is a peer correspondence channel that ships inert and activates only when a home's gitignored `.env` carries all four of `FM_LETTERBOX_REPO`, `FM_LETTERBOX_SELF`, `FM_LETTERBOX_PEER`, and `FM_LETTERBOX_TRANSPORT`; `bin/fm-letterbox.sh arm` then generates `state/letterbox.check.sh` and binds its bytes through `bin/fm-check-register.sh`, so the check sweep above executes it from a hash-validated private snapshot on the ORDINARY registered-custom-check path and `bin/fm-watch.sh` needs no letterbox-specific case.
The poll stashes each peer letter to `state/letterbox/inbox/<id>.json`, prints one compact line whose `letterbox` verb routes the handling turn to the `letterbox-correspondence` skill, and only then claims the id atomically, so the marker that suppresses a future announcement can never exist before the announcement it suppresses; announcement is consequently at-least-once and its consumers are idempotent on card id; a letter that cannot be answered in that turn becomes an ordinary task with ordinary metadata rather than entering a parallel store, so it is inventoried at session start and counted by the supervision predicate like any other work.
That predicate is the one place the letterbox adds a term: `bin/fm-supervision-lib.sh` treats `state/letterbox.check.sh` as needing supervision for the same reason it already treats `state/x-watch.check.sh` that way, because an idle home would otherwise arm no watcher and an unanswered letter would wait for the next session.
[letterbox.md](letterbox.md) owns operator setup, the card grammar, the credential-refusal and repository-visibility preconditions, and the crash matrix.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, Relay polling, or letterbox polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
The launch itself is typed into a pane the runtime backend created, so the agent is a child of that backend rather than of whoever called `fm-spawn.sh`.
A caller that must be the launched process's parent - to confine, trace, or account for it - uses the single [launch-prefix seam](launch-prefix.md), which owns that contract and its intended consumers.
