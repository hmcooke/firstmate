...
That declaration also outranks a run-step that is only monitoring CI when the latest recognized CI event affirmatively reports waiting or ready, including a checks-green or no-CI monitor promoted to ready: while the pipeline holds a pushed PR open until it is merged or closed, the crew has nothing left to drive, so `bin/fm-crew-state.sh` reports the declared wait as `paused` from the run-step and the status log together, retains the wait reason and PR-ready facts in its detail, and applies the same bounded cadence instead of a possible-wedge escalation every wedge window.
