...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while the [Herdr backend contract](herdr-backend.md#native-agent-state-submit-confirmation) owns that adapter's confirmation signals, narrow composer exclusions, and active limits.
