...
The poll stashes each peer letter to `state/letterbox/inbox/<id>.json`, prints one compact line whose `letterbox` verb routes the handling turn to the `letterbox-correspondence` skill, and only then claims the id atomically, so the marker that suppresses a future announcement can never exist before the announcement it suppresses; announcement is consequently at-least-once and its consumers are idempotent on card id.
A letter that cannot be answered in that turn becomes an ordinary backlog task rather than entering a parallel store, so it is inventoried at session start, while dispatched work also carries ordinary task metadata.
