...
| `fm-session-lock-lib.sh` | Shared session-lock owner identity (harness ancestry walk, holder liveness, service-owner record) for fm-lock.sh and the Claude Stop auto-arm |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, optionally through a caller-supplied wrapper ([docs/launch-prefix.md](launch-prefix.md)) |
...
| `fm-lock.sh`             | Per-home firstmate session lock, for a harness session or a declared service owner (docs/service-owner-lock.md) |
