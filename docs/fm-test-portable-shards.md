...
The remainder is the largest block of CI work and it grows whenever a test lands outside the proven-isolated set.
On green CI run [33024975161](https://github.com/hmcooke/firstmate/actions/runs/33024975161) it accumulated 40m04s of script time across 116 scripts.
...
| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of5` | 22 | 480885 ms (~8.01 min) |
| `portable-serial-2of5` | 23 | 480877 ms (~8.01 min) |
| `portable-serial-3of5` | 24 | 480875 ms (~8.01 min) |
| `portable-serial-4of5` | 23 | 480873 ms (~8.01 min) |
| `portable-serial-5of5` | 24 | 480885 ms (~8.01 min) |
| imbalance | | 12 ms |

The single longest script, `tests/fm-pr-check-security.test.sh` at 226928 ms, is the floor for any shard count.

`n` is sized from the measured remainder rather than the script count.
Four shards put a balanced shard at 10.0 minutes and its worst measured composite at 11.4 minutes, leaving under 4 minutes below the 15-minute job cap and breaching a 12-minute working ceiling as soon as the remainder reaches 48 minutes.
Five shards put a balanced shard at 8.0 minutes and its worst measured composite at 9.5 minutes, and hold that ceiling until the remainder reaches 60 minutes.
...
| portable serial 1-5 | job `timeout-minutes: 15` | Each balanced shard is about eight minutes, leaving roughly 2x hang-tripwire margin. |
