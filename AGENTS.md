...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead/unknown read of each task's recorded backend endpoint, where unknown means the backend could not be asked and is never a reason to recover.
...
3. For `check:`, act on the named poll result, including merges, Relay events, `letterbox` peer letters, and process-to-event source results.
...
- `letterbox-correspondence` - load on any `check:` wake whose line starts with the `letterbox` verb, before replying to or sending a peer letter, and before closing a letter this estate sent; relevant only when the letterbox is armed in this home.
