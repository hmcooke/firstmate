...
The one pending composer the daemon acts on is its own unsent envelope, which it resubmits with Enter alone so a swallowed Enter cannot end delivery; anything else pending is left untouched, and `docs/herdr-backend.md` "Own-unsent recovery" owns that contract.
...
For herdr, normal idle-baseline submits are confirmed first by native agent-state showing a real turn started, then by affirmative composer clearance only when that transition was missed; the shared classifier remains the affirmative-empty pre-injection guard and conservative fallback for non-idle or unreadable baselines.
...
- **Verified type-once submit model** - the digest is typed once (`send-keys -l`
  ...
  For herdr's normal idle-baseline path it means native agent-state observed a real turn start or a post-poll composer read affirmatively proved the pending text cleared; herdr uses the shared classifier for the pre-injection composer guard and fallback paths.
